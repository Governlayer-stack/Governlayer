...
def audit_ai_system(
    ...
    prompt = f"""You are a world class AI governance auditor. Audit this system:
System: {system_name}
Description: {system_description}
Industry: {industry}

Audit against NIST AI RMF, EU AI Act, ISO 42001, GDPR and OWASP AI.
For each framework provide compliance status, gaps found and recommendations."""
    response = llm.invoke(prompt)
    return response.content
...
def get_framework_details(framework_name: str) -> str:
    ...
    prompt = f"Give a comprehensive compliance guide for {framework_name}: {FRAMEWORKS[framework_name]}. Include requirements, who it applies to, penalties and implementation steps."
    response = llm.invoke(prompt)
    return response.content

if __name__ == "__main__":
    print("GovernLayer MCP Server starting...")
    print("25 governance frameworks loaded")
    print("5 tools available to Claude")
...
def analyze_policy_gaps(
    ...
    prompt = f"""You are a senior AI governance consultant. Analyze policy gaps for:

Company: {company_name}
Industry: {industry}
Existing Policies: {existing_policies}

Compare against NIST AI RMF, EU AI Act, ISO 42001, GDPR, HIPAA, SOC2 and OWASP AI.

For each framework identify:
1. What policies already exist and are covered
2. Critical gaps that must be addressed immediately
3. Recommended new policies to create
4. Priority level: CRITICAL / HIGH / MEDIUM / LOW

Format as a professional gap analysis report."""
    response = llm.invoke(prompt)
    return response.content

@mcp.tool()
def map_jurisdiction_requirements(
    countries: str,
    industry: str,
    ai_system_type: str
) -> str:
    """Map which AI regulations apply based on countries and industry"""
    prompt = f"""You are an expert in global AI regulation. Map requirements for:
...
1. Applicable AI regulations and laws
2. Compliance deadlines
3. Penalties for non-compliance
4. Required certifications or registrations
5. Data residency requirements

Be specific with law names, article numbers and deadlines."""
    response = llm.invoke(prompt)
    return response.content
...
def generate_incident_response_plan(
    ...
    prompt = f"""You are a world class AI incident response expert. Generate a response plan:
...
Create a detailed incident response plan including:
1. Immediate containment steps (first 1 hour)
2. Assessment and investigation steps
3. Regulatory notification requirements and deadlines
4. Stakeholder communication templates
5. Recovery and remediation steps
6. Post-incident review process
7. Relevant frameworks: NIST AI RMF, EU AI Act, ISO 27001

Make it actionable with specific timelines."""
    response = llm.invoke(prompt)
    return response.content
...
def track_compliance_deadlines(region: str) -> str:
    ...
    prompt = f"""You are an AI regulatory expert. Based on these search results:

{results}

Create a compliance deadline tracker for {region} including:
1. Regulation name
2. Deadline date
3. Who it applies to
4. What must be done
...
Sort by urgency with most urgent first."""
    response = llm.invoke(prompt)
    return response.content

@mcp.tool()
def analyze_ai_threats(
    system_type: str,
    ...
    prompt = f"""You are an AI security expert specializing in MITRE ATLAS and OWASP AI Security.
...
1. Most likely attack vectors for this system type
2. MITRE ATLAS techniques that apply
3. OWASP AI Top 10 risks that apply
4. Specific vulnerabilities to watch for
