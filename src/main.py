...
from src.api import (
    achonye, admin, agent_governance, agent_registry, agi, analytics, analytics_usage,
    assets, audit, auth, automation, billing, calendar, compliance_hub, controls, credentials,
    credit, dashboard, demo, enterprise, enterprise_features, erg, evidence, export, frameworks,
    fraud, governance, growth, hitl, incidents, integrations, knowledge_graph, ledger, lineage,
    mfa, notifications, oauth, ipi, personnel, pii, policies, privacy, rbac_views, redteam,
    registry, remediation, reports, residency, risk, safety, sso, threats, v1, vendor_risk,
    webhooks, workspace,
)
...
def create_app() -> FastAPI:
    ...
    def admin_send_email(payload: dict):
        """Send a custom email via Resend. Requires admin_key in payload."""
        key = payload.get("key", "")
        if not settings.admin_key or key != settings.admin_key:
            raise HTTPException(status_code=403, detail="Invalid admin key")
        to = payload.get("to", "")
        ...
        ok = _send(to, subj, body)
        return {"sent": ok, "to": to, "subject": subj}
